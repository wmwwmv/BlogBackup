...
english_title: seafile-install-configration
top: 0
date:
categories: 
tags: 
...
```
创建两个目录，建议放在root目录之外，防止因为权限产生的错误，给775权限，文件夹所有者ubuntu，一个用来放程序文件，另一个用来放数据。这里的目录分别为seafile和cloud。
```bash
sudo -i
mkdir cloud seafile
passwd ubuntu
...
su ubuntu
chown ubuntu cloud
chown ubuntu seafile
chmod 775 cloud seafile
```
...
```
第一步，配置ccnet，填yes，因为数据库刚才配置过了，下面是存储数据挂载点，这里填的是`/cloud`，再下面默认即可。
...
这里是我的配置
<script src="https://gist.github.com/wmwwmv/65f48a41cd9d2b19903d2bda891bb794.js"></script>
### 开机启动
参考[官方文档](https://manual-cn.seafile.com/deploy/start_seafile_at_system_bootup.html)
这里是我的三个配置文件
- seafile.service
<script src="https://gist.github.com/wmwwmv/939a8ba5540b225bf919f07c15ca0935.js"></script>
- seahub.service
<script src="https://gist.github.com/wmwwmv/b267fff4fbf26a8a5e157de8408419d2.js"></script>
- seafile-client.service
<script src="https://gist.github.com/wmwwmv/e749705314eaab79df9a49bba7d20e36.js"></script>
**Ubuntu 14.10 以及更旧的版本**
<script src="https://gist.github.com/wmwwmv/416e6fe03d3f7c735e660a6a6d12cafb.js"></script>
##错误调试
